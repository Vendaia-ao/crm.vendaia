        ...
        <div className="flex items-center gap-3">
          <div className="flex bg-slate-100 p-0.5 rounded-lg border border-slate-200">
            <button
              ...
              className={`p-1.5 rounded transition ${viewMode === 'kanban' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
              title="Vista Kanban"
            >
            ...
            <button
              ...
              className={`p-1.5 rounded transition ${viewMode === 'table' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
              title="Vista Tabela"
            >
          ...
          {listEstados.map(etapa => {
            ...
            return (
              <div
                ...
                className="w-80 shrink-0 bg-slate-100/50 p-3 rounded-2xl border border-slate-200 min-h-[500px] flex flex-col"
              >
                <div className={`p-2.5 rounded-xl mb-3 flex justify-between items-center bg-white border ${estadoBadge(etapa)}`}>
                  <span className="text-[11px] font-black uppercase tracking-widest">{etapa}</span>
                  <span className="text-[10px] font-bold bg-white/50 px-2 py-0.5 rounded-full">{cols.length}</span>
                </div>
                ...
                <div className="flex flex-col gap-3 flex-1 overflow-y-auto">
                  {cols.map(proj => {
                    ...
                    return (
                      <div
                        ...
                        className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm hover:shadow transition cursor-grab active:cursor-grabbing text-left group"
                      >
                  ...
                  {cols.length === 0 && (
                    <div className="text-center p-4 text-[11px] font-semibold text-slate-400 border-2 border-dashed border-slate-200 rounded-xl">
          ...
          <div className="p-3 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <select 
                ...
                className="text-[10px] border border-slate-200 rounded p-1 focus:outline-none"
              >
                  ...
                  return (
                    <tr key={proj.id} className="hover:bg-slate-50/50 transition">
                      ...
                      <td className="px-5 py-3 text-center">
                        <span className={`px-2 py-1 rounded text-[9px] uppercase font-black tracking-wider border ${estadoBadge(proj.estado)}`}>
                          {proj.estado}
                        </span>
                      </td>
                      <td className="px-5 py-3 text-right">
                        <div className="relative inline-block text-left">
                          <button onClick={() => setOpenDropdownId(isDropdownOpen ? null : proj.id)} className="p-1.5 text-slate-400 hover:bg-slate-100 rounded-lg">
                          ...
                          {isDropdownOpen && (
                            <>
                              ...
                              <div className="absolute right-0 mt-1 w-48 bg-white border border-slate-200 rounded-xl shadow-xl z-20 py-1 flex flex-col text-left font-semibold text-xs">
                                <button onClick={() => { setOpenDropdownId(null); setEditingProj(proj); setEditPrazo(proj.prazo); setEditObs(proj.observacoes || ''); }} className="px-4 py-2 hover:bg-slate-50 text-slate-700 flex items-center gap-2">
                                ...
                                <div className="px-4 py-2 hover:bg-slate-50 text-slate-700 flex flex-col gap-1 border-t border-slate-100 mt-1 pt-2">
                                  ...
                                  <select
                                    value={proj.estado}
                                    onChange={(e) => { setOpenDropdownId(null); onUpdateProjectoStatus(proj.id, e.target.value as EstadoProjecto); }}
                                ...
                                <button onClick={() => { setOpenDropdownId(null); if(confirm('Eliminar projecto?')) onDeleteProjecto(proj.id); }} className="px-4 py-2 mt-1 border-t border-slate-100 hover:bg-red-50 text-red-600 flex items-center gap-2">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-slate-100 overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleEditSubmit} className="p-5 space-y-4 text-xs font-semibold">
              <div>
                ...
                <input type="date" required value={editPrazo} onChange={e => setEditPrazo(e.target.value)} className="w-full px-3 py-2 border border-slate-200 bg-slate-50 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-400" />
              </div>
              <div>
                <label className="block text-[10px] text-slate-500 uppercase mb-1">Notas de Entrega / Detalhes</label>
                <textarea rows={4} value={editObs} onChange={e => setEditObs(e.target.value)} className="w-full px-3 py-2 border border-slate-200 bg-slate-50 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-400 resize-none" placeholder="Ex: Cliente solicitou mudança nas cores..."/>
              </div>
              <div className="flex justify-end pt-2 border-t border-slate-100">
                <button type="submit" className="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold transition">Guardar Modificações</button>
              </div>
            </form>
          </div>
        </div>
            ...
            <form onSubmit={handleCreateSubmit} className="p-5 space-y-4 text-xs font-semibold max-h-[80vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2 sm:col-span-1">
                  ...
                  <select required value={newResponsavel} onChange={e => setNewResponsavel(e.target.value)} className="w-full px-3 py-2 border border-slate-200 bg-slate-50 rounded-xl">
                ...
                <div className="col-span-2 sm:col-span-1">
                  ...
                  <input type="date" required value={newPrazo} onChange={e => setNewPrazo(e.target.value)} className="w-full px-3 py-2 border border-slate-200 bg-slate-50 rounded-xl" />
                </div>
              </div>

              <div>
                <label className="block text-[10px] text-slate-500 uppercase mb-1">Observações de Produção</label>
                <textarea rows={3} value={newObs} onChange={e => setNewObs(e.target.value)} className="w-full px-3 py-2 border border-slate-200 bg-slate-50 rounded-xl resize-none" placeholder="..." />
              </div>

              <div className="flex justify-end pt-3 border-t border-slate-100">
                <button type="submit" className="px-5 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-bold transition">Adicionar Projecto</button>
              </div>
            </form>
          </div>
        </div>
