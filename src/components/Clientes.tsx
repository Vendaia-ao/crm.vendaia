  ...
  return (
    <div id="clientes-module" className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-none border border-slate-200 shadow-sm p-5 flex items-center gap-4">
          <div className="w-11 h-11 rounded-none bg-blue-100 flex items-center justify-center shrink-0">
        ...
        <div className="bg-white rounded-none border border-slate-200 shadow-sm p-5 flex items-center gap-4">
          <div className="w-11 h-11 rounded-none bg-emerald-100 flex items-center justify-center shrink-0">
        ...
        <div className="bg-white rounded-none border border-slate-200 shadow-sm p-5 flex items-center gap-4">
          <div className="w-11 h-11 rounded-none bg-amber-100 flex items-center justify-center shrink-0">
        ...
        <div className="flex flex-wrap items-center gap-3 flex-1">
          <div className="relative flex-1 min-w-[200px] max-w-xs">
            ...
            <input type="text" placeholder="Pesquisar..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
              className="w-full pl-9 pr-3 py-1.5 text-xs bg-slate-50 border border-slate-200 rounded-none focus:outline-none focus:ring-1 focus:ring-blue-400" />
          </div>
          <div className="flex items-center gap-1.5">
            <span className="text-[10px] text-slate-400 font-bold uppercase">Estado:</span>
            <select value={estadoFilter} onChange={e => setEstadoFilter(e.target.value)}
              className="bg-slate-50 border border-slate-200 text-xs text-slate-700 font-semibold rounded-none px-2.5 py-1.5 focus:outline-none focus:ring-1 focus:ring-blue-400">
      ...
      {clientes.length > 0 && (
        <div className="bg-white rounded-none border border-slate-200/80 shadow-sm overflow-hidden text-left flex flex-col">
          {/* Table Toolbar */}
                ...
                {paginatedClientes.map(c => {
                  ...
                  return (
                    <tr key={c.id} className="hover:bg-slate-50/70 transition group">
                      ...
                      <td className="px-5 py-4">
                        <span className="bg-blue-50 text-blue-700 font-semibold px-2 py-0.5 rounded-none text-[10px] uppercase border border-blue-100">
                      ...
                      <td className="px-5 py-4 text-center">
                        <span className={`inline-block px-2.5 py-1 rounded-none text-[9px] uppercase border tracking-wider font-bold ${estadoBadge(c.estado)}`}>
                          {c.estado}
                        </span>
                        {c.data_reuniao && (
                      ...
                      <td className="px-5 py-4">
                        {c.proxima_acao ? (
                          <span className="bg-orange-50 text-orange-700 font-extrabold px-2 py-1 rounded-none text-[10px] border border-orange-100">{c.proxima_acao}</span>
                        ) : <span className="text-[10px] text-slate-300 italic">—</span>}
                      </td>
                      <td className="px-5 py-4 text-right">
                        <div className="relative inline-block text-left">
                          <button onClick={(e) => toggleDropdown(c.id, e)} className="p-1.5 text-slate-400 hover:bg-slate-100 rounded-none">
                          ...
                          {isDropdownOpen && dropdownPos && (
                            <>
                              ...
                              >
                                <button onClick={() => { setOpenDropdownId(null); setDropdownPos(null); openMeetingModal(c); }} className="px-4 py-2 hover:bg-purple-50 text-purple-700 flex items-center gap-2">
                                  <Calendar className="w-4 h-4" /> Agendar Reunião
                                </button>
                                <button onClick={() => { setOpenDropdownId(null); setDropdownPos(null); openDetails(c); }} className="px-4 py-2 hover:bg-slate-50 text-slate-700 flex items-center gap-2">
                                  <Star className="w-4 h-4" /> Alterar Relação
                                </button>
                                <button onClick={() => { setOpenDropdownId(null); setDropdownPos(null); if(confirm('Eliminar cliente?')) onDeleteCliente(c.id); }} className="px-4 py-2 mt-1 border-t border-slate-100 hover:bg-red-50 text-red-600 flex items-center gap-2">
      ...
      {meetingCliente && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 text-left" onClick={() => setMeetingCliente(null)}>
          <div className="bg-white rounded-none shadow-2xl w-full max-w-sm border border-slate-100 overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={saveMeeting} className="p-5 space-y-4 text-xs font-semibold">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="date" required value={dataReuniao} onChange={e => setDataReuniao(e.target.value)} className="w-full px-3 py-2 border border-slate-200 bg-slate-50 rounded-none" />
                </div>
                <div>
                  <label className="block text-[10px] text-slate-500 uppercase mb-1">Hora</label>
                  <input type="time" required value={horaReuniao} onChange={e => setHoraReuniao(e.target.value)} className="w-full px-3 py-2 border border-slate-200 bg-slate-50 rounded-none" />
                </div>
              </div>
              <div>
                <label className="block text-[10px] text-slate-500 uppercase mb-1">Local / Link Virtual</label>
                <input type="text" value={localReuniao} onChange={e => setLocalReuniao(e.target.value)} className="w-full px-3 py-2 border border-slate-200 bg-slate-50 rounded-none" placeholder="Ex: Google Meet" />
              </div>
              <div>
                <label className="block text-[10px] text-slate-500 uppercase mb-1">Notas Internas</label>
                <textarea rows={2} value={obsReuniao} onChange={e => setObsReuniao(e.target.value)} className="w-full px-3 py-2 border border-slate-200 bg-slate-50 rounded-none resize-none" placeholder="Ex: Levar portfólio..." />
              </div>
              <div className="flex justify-end pt-3 border-t border-slate-100">
                <button type="submit" className="px-5 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-none text-xs font-bold transition">Confirmar Agendamento</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="bg-white rounded-none shadow-2xl w-full max-w-sm border border-slate-100 overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={saveDetails} className="p-5 space-y-4 text-xs font-semibold">
              <div>
                ...
                <select value={tempEstado} onChange={e => setTempEstado(e.target.value as EstadoCliente)} className="w-full px-3 py-2 border border-slate-200 bg-slate-50 rounded-none">
                  {ESTADOS.map(est => <option key={est} value={est}>{est}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-[10px] text-slate-500 uppercase mb-1">Próxima Ação Comercial</label>
                <select value={tempAcao} onChange={e => setTempAcao(e.target.value as ProximaAcaoComercial)} className="w-full px-3 py-2 border border-slate-200 bg-slate-50 rounded-none">
                  {ACOES.map(ac => <option key={ac} value={ac}>{ac || 'Nenhuma'}</option>)}
                </select>
              </div>
              <div className="flex justify-end pt-3 border-t border-slate-100">
                <button type="submit" className="px-5 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-none text-xs font-bold transition">Guardar Alterações</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 text-left" onClick={() => setShowAddModal(false)}>
          <div className="bg-white rounded-none shadow-2xl w-full max-w-sm border border-slate-100 overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleCreateSubmit} className="p-5 space-y-4 text-xs font-semibold">
              ...
              <div className="flex justify-end pt-3 border-t border-slate-100">
                <button type="submit" className="px-5 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-none text-xs font-bold transition">Gravar Cliente</button>
              </div>
            </form>
          </div>
        </div>
